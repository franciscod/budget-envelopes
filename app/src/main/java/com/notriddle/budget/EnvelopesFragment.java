...
public class EnvelopesFragment extends Fragment
    ...
    @Override public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.newEnvelope_menuItem) {
            SQLiteDatabase db = (new EnvelopesOpenHelper(getActivity()))
                                .getWritableDatabase();
            ContentValues values = new ContentValues();
            values.put("name", "");
            values.put("color", 0);
            long itemId = db.insert("envelopes", null, values);
            db.close();
            getActivity().getContentResolver().notifyChange(EnvelopesOpenHelper.URI, null);
            openEnvelope((int)itemId);
            return true;
        } else if (id == R.id.transfer_menuItem) {
            DialogFragment f = TransferFragment.newInstance();
            f.show(getFragmentManager(), "dialog");
            return true;
        } else if (id == R.id.paycheck_menuItem) {
